...
const tierAccent: Record<string, { ring: string; badge: string }> = {
  starter: { ring: "ring-[rgba(214,177,94,0.28)]", badge: "bg-[rgba(214,177,94,0.16)] text-[rgb(5,8,20)]" },
  pro: { ring: "ring-[rgba(11,18,36,0.18)]", badge: "bg-[rgba(11,18,36,0.08)] text-[rgb(5,8,20)]" },
  business: { ring: "ring-[rgba(184,141,47,0.22)]", badge: "bg-[rgba(184,141,47,0.14)] text-[rgb(5,8,20)]" },
  custom: { ring: "ring-[rgba(11,18,36,0.14)]", badge: "bg-[rgba(11,18,36,0.06)] text-[rgb(5,8,20)]" },
};

export default function PricingPage() {
  const tiers = packages.filter((p) => p.id !== "management");
  const management = packages.find((p) => p.id === "management");

  return (
    <main className="bg-background">
      <div className="mx-auto max-w-7xl px-6 py-16">
        <p className="text-xs font-semibold tracking-widest text-[rgb(184,141,47)]">PRICING</p>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight md:text-5xl">Clear starting points</h1>
        <p className="mt-5 max-w-2xl text-muted-foreground">
          Final pricing depends on pages, features, and how custom you want the build. If you’re not sure which tier fits,
          send me your business type + timeline and I’ll recommend the best route.
        </p>
          ...
          {management ? (
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm md:col-span-2">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <div className="text-lg font-semibold tracking-tight text-foreground">{management.title}</div>
                  <div className="mt-2 text-sm text-muted-foreground">{management.description}</div>
                </div>
